...
<div class="row">               
<div class="col-md-4">
<h2><img src="/icons/newsadd.svg" style="height: 39px; width: 57px;">Edit Quotation</h2>
</div>
</div>
<br>
            ...
            <div class="panel-footer" style="margin-left: 4%;margin-top:5%">
                <div class="row">
                   ...
                   <input type="text" class="form-control" value=quotation.total_cost readonly="" style="border-radius:0px" id="total_amount" v-model="quotation.total_cost">
                    ...
                    <div class="col-md-1">
                        <button type="submit" class="btn save" name="action" value="draft" style="background: #41D080" @click="$v.quotation.$touch,chececkAction('draft')" title="Edit as a draft">Save as Draft&nbsp;<i class="fas fa-file-alt"></i></button>
                    </div>&nbsp;
                    <div class="col-md-1" style="margin-left: 4%;">
                        <button type="submit" class="btn btn-primary save" name="action" value="save"  @click="$v.quotation.$touch,chececkAction('save')" title="Save quotation">Save&nbsp;<i class="fa fa-check"></i></button>
                    </div>
                </div>
...
export default {
  ...
  quotation:{
    ...
    methods:{
            ...
            var app=this;
            this.$http({
            url: `quotationsingle/`+this.$route.params.id,
            method: 'GET',
            })
            .then((res) => {
            app.quotation= res.data.quotation;
            for (var propName in app.quotation) { 
                    if (app.quotation[propName] === null || app.quotation[propName] === undefined) {
                        app.quotation[propName] = '';
                    }
                }
            app.autoUpdateCountryCurrency();
            app.quotation.quotation_array= res.data.qp;
            console.log(res);
            
            $.each(res.data.qp,function(index, value) {
                 console.log(index);
                
                app.autoUpdateMaterialDetail(index)
                app.autoUpdateindex(index); 
            });
            app.count = this.quotation.quotation_array.length;
              ...
              getUserCustomer() {
               ...
               totalPriceCalculation() {
                ...
                updateQuotation() {
                    ...
                    this.$http.post ( 'updatequotation/'+this.$route.params.id, formData ).then(function (res) {
